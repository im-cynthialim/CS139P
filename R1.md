...
## Enumerations
- associated values: store values of other types along with case values
    - similar to discriminated unions, tagged unions, or variants
    e.g.
    ```swift
    ...
    // enum type of barcode with either a value of upc with assoc. values of type (Int, Int...) or value of qrCode with assoc. value of type String

    var newCode = Barcode.upc(1, 2, 3, 4)
    newCode = .qrCode("1234")
    ```

- can check different types of enum with switch statement
    ```swift
    // extract assoc. values as constants or vars
    switch newCode {
        case .upc (let firstInt, let secondInt, ...):
        case .qrCode ...
    }

    // if all assoc. values extracted as const/var, use single let/var
    case let .upc(firstInt, secondInt, ...) 

    // matching only one case of enumeration, checks if productBarcode matches pattern of .qrCode
...
- don't need separate interface + implementation files
- define struct/class in single file + extenral interface automatically available to other code
- object: instance of a struct/class

- struct vs. class
    - both:
        - define properties and methods
        - define subscripts to access their values
        - define initializers
        - can be extended
        - conform to protocols
    - only classes have:
        - inheritance
        - type casting: check + interpret type of a class instance at runtime
        - deinitializers to free up resources
        - reference counting: allow 1+ reference to class instance 

### Structures and Enumerations are Value Types
- value type: type whose value is copied when assigned to var/const or passed to func
- all basic types in Swift (int, float, bool, string, arrays, dicts) are value types
- structs + enums are value types

## Properties
- computed properties provided by classes, structs, enums
- stored properties provided by classes + structs
- type properties: properties associated with type
- instance properties: belong to instance of a type, every new instance gets its own set of proprty values
- can add property observers to stored props
- property wrapper: reuse code in getter/setter of multiple properties

### Type Properties
- useful for defining values that are universal to all instances of a type
- stored type properties can be vars/consts computed type props are always var props
    - always need a default value
    

- written as part of type's definition, define with `static`
    - for computed type properties, use `class` to allow for overriding
    ```swift
    // same syntax for type props in enums and classes
    // these computd type props are read-only
    struct SomeStructure {
        static var storedTypeProp = ""
...
- querying and setting type properties
    - use dot syntax
        ...
        SomeStruct.storedTypeProp = ""
        ```

e.g. 
```swift
struct AudioChannel {
    // audio threshold lvl for all AudioChannel instances
    static let thresholdLevel = 10

    // tracks max input across all channels, updated by currentLevel
...
## Methods
- funcs associated with a certain type
- classes, structs, enums can define instance methods - encapsulates tasks for an instance of a type
    - can also define type methods, associated with type itself

#### Modifying Value Types from Within Instance Methods
- in structs and enums, by default, can't modify properties of value type within instance methods
    - can use method to mutate properties, use `mutating`
        - can't use on constants
    ```swift
       struct Point {
            var x = 0.0, y = 0.0
            // moves Point instance by an amount
            // doesn't return a new point, modifies the point that it's called on
            // mutating allows the property to be modified
    ...
    ```
    - method can also assign a new instance with `self` property + update existing instance with it

### Type Methods
- use `static` in front of method
- classes can use `class` to allow overriding
- call type methods with dot syntax
- `self` property available within body of type method, refers to the type instead of instance


```swift
struct LevelTracker {
    // trcks level that player is on and unlocks next levels
    static var highestUnlockedLevel = 1
    var currentLevel = 1

    // two type functions defined, both can access highestUnlockedLevel without dot syntax

    // updates highest level when new one unlocked
    ...
    // instance method
    // use discardableResult since it's ok to ignore return value
    @discardableResult
...
// used with Player class
class Player {
    // creates instances of LevelTracker per player
...
- customize with input params, optional property types, assign constant properties

- initialization params
    - similar to func/method params
    ```swift
    struct Celsius {
        var tempInCels: Double
        init(from Fahren fahren: Double) {
            tempInCels = fahren...
        }
...
- parameter names and arg labels
    - initializiers don't have function names before `()` like funcs/methods
    - Swift gives automatic arg label for every param in init if you don't give one
    ```swift
        struct Color {
...
- initalizer params without arg labels
    - use `_` if you don't want arg labels
    ```swift
        struct Celsius {
...
- optional property types have `nil` as default init value

- assigning constant properties during initialization
    - can assign value to const anytime during init but needs definite value when init finishes
    - for classes, constant property can only be modified during init of class, can't be modified by subclass